...
fn test_e2e_config_has_device_id() {
    ...
    assert!(content.contains("device_id="), "Config should have device_id");
    
    // Device ID should not be empty
    let device_id_line = content.lines()
...
fn test_e2e_toggle_changes_state() {
    ...
    let status2 = Command::new(get_cli_path())
        .arg("status")
        .output()
        .expect("Failed to run status");
...
fn test_e2e_scan_safe_package() {
    ...
    assert!(
        ...
        "lodash should be scanned as safe: {}", stdout
    );
}
...
fn test_e2e_scan_with_version() {
    ...
    assert!(
        ...
        "Should scan specific version: {}", combined
    );
}
...
fn test_e2e_monitor_requires_pro() {
    ...
    assert!(
        ...
        "Should mention Pro or enable monitoring: {}", combined
    );
}
...
fn test_e2e_handles_invalid_package_name() {
    ...
    // Should not panic
    assert!(!stderr.contains("panic"), "Should not panic on invalid package");
}
