    ...
    match api::scan_package(package) {
        Ok(response) => {
            ...
            if !pro_scanned {
                ...
                println!("  {}", "| PRO features not scanned:                       |".bright_black());
                println!("  {}", "|    * Version Anomaly Detection                  |".bright_black());
                println!("  {}", "|    * Script Analysis                            |".bright_black());
                println!("  {}", "|    * Behavioral Sandbox                         |".bright_black());
                println!("  {}", "|    * Quarantine Alert                           |".bright_black());
                println!("  {}", "|    Upgrade: sapo upgrade                        |".bright_black());
                println!("  {}", "+-------------------------------------------------+".bright_black());
            } else {
                ...
                if response.sandbox_cached.unwrap_or(false) {
                    ...
                    match sandbox_risk {
                        "dangerous" => {
                            println!("  {} Sandbox: DANGEROUS (score: {})", "[!]".red(), sandbox_score);
                        }
                        "warning" => {
                            println!("  {} Sandbox: Warning (score: {})", "[!]".yellow(), sandbox_score);
                        }
                        _ => {
