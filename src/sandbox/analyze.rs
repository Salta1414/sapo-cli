...
pub fn run_local_sandbox(package: &str, version: &str) {
    if !is_sandbox_available() {
        ...
        #[cfg(target_os = "linux")]
        println!("     {} Install strace: sudo apt install strace", "Tip:".bright_black());
    ...
    match api::analyze_sandbox(&request) {
        Ok(response) => {
            ...
            match risk_level {
                "dangerous" => {
                    println!("  {} Sandbox: DANGEROUS behavior detected (score: {})", "[!]".red(), score);
                }
                "warning" => {
                    println!("  {} Sandbox: Suspicious behavior (score: {})", "[!]".yellow(), score);
                }
                _ => {
...
fn collect_behavior(package: &str, version: &str) -> Option<SandboxBehavior> {
    ...
    fs::write(&package_json, r#"{"name":"sapo-sandbox-test","version":"1.0.0","private":true}"#).ok()?;
...
fn collect_behavior_linux(sandbox_dir: &PathBuf, pkg_spec: &str) -> Option<SandboxBehavior> {
    ...
    let status = Command::new("timeout")
        .args([
            ...
            "-o", strace_log.to_str()?,
            "npm", "install", pkg_spec, "--ignore-scripts=false"
        ])
        ...
        for line in reader.lines().flatten() {
            ...
            if line.contains("open") {
                if let Some(file_path) = extract_quoted_string(&line) {
                    ...
                    if file_path.contains(".ssh") || file_path.contains(".aws") || file_path.contains(".npmrc") {
            ...
            if let Ok(content) = fs::read_to_string(&entry) {
                for (pattern, description) in &patterns {
                    if content.contains(pattern) {
                        ...
                        match *description {
                            "process.env access" => {
                                if !behavior.env_vars_accessed.contains(&description.to_string()) {
...
fn walkdir(path: &PathBuf) -> std::io::Result<Vec<PathBuf>> {
    ...
    if path.is_dir() {
        for entry in fs::read_dir(path)? {
            ...
            } else if path.extension().map(|e| e == "js" || e == "ts" || e == "mjs" || e == "cjs").unwrap_or(false) {
                result.push(path);
            }
        }
    }
